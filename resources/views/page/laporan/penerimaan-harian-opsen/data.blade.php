    ...
    <div class="col px-3">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Daftar Penerimaan Harian Opsen {{ $wilayah->nm_wilayah }} Tanggal
                    {{ $tanggal }}</h4>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                            <div class="dropdown-item">
                                <form action="{{ route('penerimaan-harian-opsen.pdf') }}" method="post" target="_blank">
                                    @csrf
                                    <input type="hidden" id="tanggal" name="tanggal" value="{{ $tanggal }}">
                                    <input type="hidden" id="kd_wilayah" name="kd_wilayah"
                                        value="{{ $wilayah->kd_wilayah }}">
                                    <button type="submit" class="btn btn-sm btn-primary w-100">PDF</button>
                                </form>
                            </div>
                            <div class="dropdown-item">
                                <form action="{{ route('penerimaan-harian-opsen.excel') }}" method="post" target="_blank">
                                    @csrf
                                    <input type="hidden" id="tanggal" name="tanggal" value="{{ $tanggal }}">
                                    <input type="hidden" id="kd_wilayah" name="kd_wilayah"
                                        value="{{ $wilayah->kd_wilayah }}">
